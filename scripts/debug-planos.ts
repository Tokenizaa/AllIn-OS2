/**
 * Script de Debug para Endpoint Planos
 * 
 * Este script faz uma requisição direta ao endpoint /simulacao-planos
 * e imprime todas as informações para debug.
 * Execute com: npx tsx scripts/debug-planos.ts
 */
...
async function debugPlanos() {
  console.log('🔍 DEBUG - Endpoint Planos (simulacao-planos)');
  console.log('========================================\n');
  ...
  try {
    ...
    // 2. Fazer requisição para /simulacao-planos
    console.log('PASSO 2: Fazendo requisição para /simulacao-planos...');
    const planosUrl = `${ALLIN_API_BASE_URL}/simulacao-planos`;
    const headers = {
      'Content-Type': 'application/json',
      'Authorization': `Bearer ${tokenData.access_token}`,
    };

    console.log(`  URL: ${planosUrl}`);
    ...
    console.log(`  Status: ${planosResponse.status}`);
    console.log(`  Status Text: ${planosResponse.statusText}\n`);

    // 3. Imprimir resposta bruta
    console.log('PASSO 3: Resposta bruta...');
    const responseText = await planosResponse.text();
    ...
    process.exit(planosResponse.ok ? 0 : 1);
  } catch (error) {
