...
export class QualificationSyncService extends BaseSyncService {
        ...
        try {
            ...
            for (const distributor of distributors) {
                try {
                    ...
                    const allinQualification = await this.executeWithRetry(() => allinService.getDistribuidorQualificacaoAtual(allinId), `Fetch qualification for distributor ${allinId} from AllIn`);
                    ...
                    this.addError(result, distributor.id, 'Failed to sync qualification', error);
     ...
     */
    async syncDistributorQualification(distributorId) {
        const result = this.createSyncResult();
        try {
