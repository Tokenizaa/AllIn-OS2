/**
 * Script de Debug para Endpoint Pedidos
 * 
 * Este script faz uma requisição direta ao endpoint /pedidos
 * e imprime todas as informações para debug.
 * Execute com: npx tsx scripts/debug-pedidos.ts
 */
...
async function debugPedidos() {
  console.log('🔍 DEBUG - Endpoint Pedidos');
  console.log('========================================\n');
  ...
  try {
    ...
    // 2. Fazer requisição para /pedidos
    console.log('PASSO 2: Fazendo requisição para /pedidos...');
    const pedidosUrl = `${ALLIN_API_BASE_URL}/pedidos`;
    const headers = {
      'Content-Type': 'application/json',
      'Authorization': `Bearer ${tokenData.access_token}`,
    };

    console.log(`  URL: ${pedidosUrl}`);
    ...
    console.log(`  Status Text: ${pedidosResponse.statusText}\n`);

    // 3. Imprimir resposta bruta
    console.log('PASSO 3: Resposta bruta...');
    const responseText = await pedidosResponse.text();
    ...
    process.exit(pedidosResponse.ok ? 0 : 1);
  } catch (error) {
