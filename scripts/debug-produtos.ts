/**
 * Script de Debug para Endpoint Produtos
 * 
 * Este script faz uma requisição direta ao endpoint /produtos
 * e imprime todas as informações para debug.
 * Execute com: npx tsx scripts/debug-produtos.ts
 */
...
async function debugProdutos() {
  console.log('🔍 DEBUG - Endpoint Produtos');
  console.log('========================================\n');
  ...
  try {
    ...
    // 2. Fazer requisição para /produtos
    console.log('PASSO 2: Fazendo requisição para /produtos...');
    const produtosUrl = `${ALLIN_API_BASE_URL}/produtos`;
    const headers = {
      'Content-Type': 'application/json',
      'Authorization': `Bearer ${tokenData.access_token}`,
    };

    console.log(`  URL: ${produtosUrl}`);
    ...
    console.log(`  Status Text: ${produtosResponse.statusText}\n`);

    // 3. Imprimir resposta bruta
    console.log('PASSO 3: Resposta bruta...');
    const responseText = await produtosResponse.text();
    ...
    process.exit(produtosResponse.ok ? 0 : 1);
  } catch (error) {
