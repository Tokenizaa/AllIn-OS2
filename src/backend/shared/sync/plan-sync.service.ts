...
import { PlanMapper, LocalPlan } from './mappers/plan.mapper';
import { SyncResult } from './dto/sync-result.dto';
import { PlanRepository } from '../../modules/plans/repositories/plan.repository';

export class PlanSyncService extends BaseSyncService<LocalPlan> {
   ...
   */
  protected mapFromAllin(data: any): LocalPlan {
  ...
  public async sync(params?: { incremental?: boolean; since?: Date }): Promise<SyncResult> {
    ...
    try {
      ...
      const allinPlans = await this.executeWithRetry(
        () => allinService.getPlanosAtivos(),
        'Fetch plans from AllIn'
      );
      ...
      await this.processAllBatches(
        localPlans,
        async (plan) => await this.processPlan(plan, params),
        result
      );
