      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="md:col-span-2 relative overflow-hidden rounded-3xl border border-primary/30 bg-gradient-to-br from-primary/20 via-fuchsia-500/10 to-transparent p-6">
          ...
          <div className="relative">
            ...
            <p className="mt-2 text-4xl md:text-5xl font-bold">{formatCurrency(available)}</p>
            <div className="mt-6 grid grid-cols-3 gap-4">
              <Mini icon={Lock} label="Bloqueado" value={formatCurrency(blocked)} />
              <Mini icon={Clock} label="A liberar" value={formatCurrency(pending)} />
              <Mini icon={TrendingUp} label="Ganho no ano" value={formatCurrency(totalYear)} />
            </div>
          </div>
        </div>
        <div className="rounded-3xl border border-border/60 bg-card/60 p-5">
          ...
          <p className="mt-1 text-2xl font-bold text-success">{formatCurrency(available + pending)}</p>
          <p className="mt-1 text-xs text-muted-foreground">Baseado no saldo real e liberações pendentes.</p>
        </div>
      </div>

      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <StatCard label="Ganho no ano" value={formatCurrency(totalYear)} delta={0} icon={Wallet} accent="success" />
        <StatCard label="Saques realizados" value={String(withdrawals.length)} delta={0} accent="info" />
        <StatCard label="Bônus pendentes" value={formatCurrency(pending)} accent="warning" hint="Saldo ainda não liberado" />
        ...
        <ul className="space-y-2">
          {withdrawals.map((entry) => (
            <li key={entry.id} className="flex items-center justify-between rounded-lg border border-border/60 bg-background/40 p-3 text-sm">
              ...
              <span className="font-semibold">{formatCurrency(Number(entry.amount || 0))}</span>
            </li>
          ))}
        </ul>
      </div>
