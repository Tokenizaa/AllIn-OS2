...
export function NetworkPage() {
  ...
  const filtered = useMemo(() => {
    ...
    return (nodes as NodeRow[]).filter((node) => {
  ...
  const total = (nodes as NodeRow[]).length;
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <Card label="Ativos" value={String((nodes as NodeRow[]).filter((n) => n.status !== "inactive").length)} icon={<TrendingUp className="h-4 w-4 text-fuchsia-400" />} />
        <Card label="Qualificados" value={String((nodes as NodeRow[]).filter((n) => String(n.qualification || "").length > 0).length)} icon={<Sparkles className="h-4 w-4 text-amber-400" />} />
        <Card label="Inativos" value={String((nodes as NodeRow[]).filter((n) => n.status === "inactive").length)} icon={<Sparkles className="h-4 w-4 text-rose-400" />} />
      </div>
