...
export function Dashboard() {
  ...
  const { stats: current, salesSeries, bonusOrigin, topProducts, timeline, aiInsights } = data as any;
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <StatCard label="Saldo disponível" value={formatCurrency(current.saldoDisponivel)} delta={0} icon={Wallet} accent="success" />
        <StatCard label="Comissão acumulada" value={formatCurrency(current.comissaoAcumulada)} delta={0} icon={Trophy} accent="primary" />
        <StatCard label="Total vendido" value={formatCurrency(current.totalVendido)} delta={0} icon={TrendingUp} accent="info" />
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-4">
        ...
        <div className="rounded-2xl border border-border/60 bg-card/60 p-5">
          ...
          <ul className="mt-4 space-y-4">
            <li className="text-xs text-muted-foreground">Metas não disponíveis no momento</li>
          </ul>
        </div>
      </div>
