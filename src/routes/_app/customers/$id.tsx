...
function Customer360() {
  ...
  const [pointsWallet, setPointsWallet] = useState<any | null>(null);
  ...
  const handleCreatePointsWallet = async () => {
    try {
      const { data: newPtsWallet, error } = await supabase
        .from("points_wallets")
        .insert({
          customer_id: c.id,
          balance: 0,
  ...
  const handleAddTransaction = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setWallet({ ...wallet, balance: balanceAfter, available_balance: balanceAfter });
  ...
  return (
    <div className="space-y-6 pb-12 animate-fade-in">
      ...
      <PageHeader
        ...
        actions={
          <>
            <Button size="sm" variant="outline" className="gap-1.5" onClick={fetch360Data}>
              Re-sincronizar
            </Button>
