...
async function resolvePlanConfig(customerId: string) {
  const { data: activePlan, error: findError } = await supabase
    .schema('mlm')
    .from('planos_distribuidores')
    .select('*')
    .eq('id_comprador', customerId)
    .eq('status', 'active')
    .maybeSingle();
  if (findError || !activePlan?.plan_id) {
    return { planKey: null as string | null, directPct: 0, sponsorPct: 0, generationBonuses: [] as Array<{ generation: number; percentage: number }>, extraDirectsBonuses: [] as Array<{ minDirects: number; percentage: number }>, planName: null as string | null, planMode: null as string | null };
  }
...
export const CommissionService = {
  ...
  async calculateMLMCommission(data: {
    order_id: string;
    seller_id: string;
    order_amount: number;
  }) {
