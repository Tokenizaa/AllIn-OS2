...
class CustomerRepository extends BaseRepository {
  ...
  async findByEmail(email) {
    const { data, error } = await this.getClient().from(this.tableName).select("*").eq("email", email).single();
    if (error) throw error;
    return data;
  }
  async findByCpf(cpf) {
    const { data, error } = await this.getClient().from(this.tableName).select("*").eq("metadata->>cpf", cpf).single();
    if (error) throw error;
    return data;
  }
  async findBySponsorId(sponsorId, options) {
    let query = this.getClient().from(this.tableName).select("*").eq("patrocinador_comprador", sponsorId);
    if (options?.limit) {
  ...
  async getCustomer360(customerId) {
    const { data, error } = await this.getClient().from("customer_360_view").select("*").eq("id", customerId).single();
    if (error) throw error;
    return data;
  }
  async getAllCustomer360(options) {
    let query = this.getClient().from("customer_360_view").select("*");
    if (options?.status) {
  ...
  async countByPlan(planId) {
    const { count, error } = await this.getClient().from(this.tableName).select("*", { count: "exact", head: true }).eq("plan_id", planId);
    if (error) throw error;
    return count || 0;
  }
}
...
const getCustomers = createServerFn({
  method: "GET"
}).inputValidator(paginationSchema.merge(filterSchema)).handler(getCustomers_createServerFn_handler, async ({
  data
}) => {
  try {
...
const createCustomer = createServerFn({
  method: "POST"
}).inputValidator(createCustomerSchema).handler(createCustomer_createServerFn_handler, async ({
  data
}) => {
  try {
