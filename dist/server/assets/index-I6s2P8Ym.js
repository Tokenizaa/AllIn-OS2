  ...
  useEffect(() => {
    (async () => {
      const [{
        ...
        data: customersData
      }] = await Promise.all([supabase.from("orders").select("id, customer_id, numero_pedido, status_pedido, payment_method, items, valor_total_pedido, valor_total, created_at").order("created_at", {
        ascending: false
      }).limit(60), supabase.from("customers").select("id, usuario, id_comprador, user_id, qualification, telefone, metadata").order("created_at", {
        ascending: false
      }).limit(250)]);
    ...
    /* @__PURE__ */ jsx("div", { className: "rounded-xl border border-border bg-card/40 overflow-hidden", children: /* @__PURE__ */ jsxs("table", { className: "w-full text-sm", children: [
      ...
      /* @__PURE__ */ jsx("tbody", { className: "divide-y divide-border/60", children: orders.map((o) => {
        ...
        return /* @__PURE__ */ jsxs("tr", { className: "hover:bg-accent/30", children: [
          ...
          /* @__PURE__ */ jsx("td", { className: "px-4 py-3", children: customer ? /* @__PURE__ */ jsx(Link, { to: "/_app/customers/$id", params: {
            id: customer.id
