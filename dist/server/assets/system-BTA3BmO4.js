...
import { c as cn, B as Button, a as Badge, b as useAuth } from "./router-Piw3VGP8.js";
import { P as PageHeader } from "./page-header-D_YhEPyH.js";
import * as React from "react";
import { useState } from "react";
import { C as Card, b as CardHeader, c as CardTitle, d as CardDescription, a as CardContent } from "./card-Cyhv06K2.js";
import { I as Input } from "./input-QP3DCRKc.js";
import { S as Switch, L as Label } from "./switch-CKkqjrTs.js";
import { T as Tabs, a as TabsList, b as TabsTrigger, c as TabsContent } from "./tabs-DBV3uj2e.js";
import { T as Table, e as TableHeader, f as TableRow, g as TableHead, h as TableBody, i as TableCell, S as Select, a as SelectTrigger, b as SelectValue, c as SelectContent, d as SelectItem } from "./select-DmczOoJR.js";
...
import { D as Dialog, e as DialogTrigger, a as DialogContent, b as DialogHeader, c as DialogTitle, d as DialogDescription, f as DialogFooter } from "./dialog-CDk5i78z.js";
...
function BonusConfiguration() {
  ...
  const handleToggleActive = (ruleId) => {
    setBonusRules(
      (rules) => rules.map((rule) => rule.id === ruleId ? { ...rule, isActive: !rule.isActive } : rule)
    );
  };
  ...
  const handleSave = (config) => {
    if (selectedRule) {
      setBonusRules(
        (rules) => rules.map((rule) => rule.id === selectedRule.id ? { ...rule, ...config } : rule)
      );
    }
    setIsEditing(false);
    setSelectedRule(null);
...
function GatewayManagement() {
  ...
  const handleToggleActive = (gatewayId) => {
    setGateways(
      (items) => items.map((gateway) => gateway.id === gatewayId ? { ...gateway, isActive: !gateway.isActive } : gateway)
    );
  };
  ...
  const handleSave = (config) => {
    if (selectedGateway) {
      setGateways(
        (items) => items.map((gateway) => gateway.id === selectedGateway.id ? { ...gateway, ...config } : gateway)
      );
    }
    setIsEditing(false);
    setSelectedGateway(null);
