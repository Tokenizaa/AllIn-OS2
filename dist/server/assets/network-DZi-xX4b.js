...
function NetworkPage() {
  ...
  useEffect(() => {
    (async () => {
      ...
      }] = await Promise.all([supabase.from("customers").select("id, usuario, id_comprador, qualification, status, cidade, estado, user_id").order("created_at", {
        ascending: false
  ...
  const data = customers.map((c) => ({
    name: (c.usuario || c.id_comprador || "D").split(" ")[0],
