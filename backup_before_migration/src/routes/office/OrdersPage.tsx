  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <StatCard label="Total comprado" value={formatCurrency(total)} delta={0} icon={ShoppingBag} accent="primary" />
        <StatCard label="Ticket médio" value={formatCurrency(ticket)} delta={0} accent="info" />
              ...
              {isLoading ? (
                ...
                filteredOrders.map((order) => (
                  <tr key={order.id} className="border-b border-border/40 hover:bg-muted/30 transition-colors">
                    ...
                    <td className="px-4 py-3 text-right font-semibold">{formatCurrency(Number(order.total_amount || 0))}</td>
