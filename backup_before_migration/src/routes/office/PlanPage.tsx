...
export function PlanPage() {
  ...
  const createdAt = (profile as any)?.created_at ? new Date((profile as any).created_at).toLocaleDateString("pt-BR") : "-";
        ...
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="lg:col-span-2 relative overflow-hidden rounded-3xl border border-primary/30 bg-gradient-to-br from-primary/15 via-fuchsia-500/10 to-transparent p-6">
          ...
          <div className="relative">
            ...
            <div className="mt-6 grid grid-cols-3 gap-4">
              ...
              <Metric label="Mensalidade" value={formatCurrency(Number(current?.price ?? 0))} />
            </div>
            <div className="mt-6">
          ...
          {planCards.map((plan, index) => {
            ...
            return (
              ...
              >
                ...
                <p className="mt-1 text-2xl font-bold">{formatCurrency(Number(plan.price || 0))}<span className="text-xs text-muted-foreground font-normal">/mês</span></p>
